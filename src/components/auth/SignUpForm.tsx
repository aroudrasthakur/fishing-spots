...
import { AuthSpinner, authCallbackUrl, messageBoxClass } from "./auth-shared";

type SignUpFormProps = {
  supabase: SupabaseClient;
};

export function SignUpForm({ supabase }: SignUpFormProps) {
  const baseId = useId();
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (error) {
      ...
      setTone("success");
      setMessage(
        "Check your email to confirm your account, then sign in from the map. (For local dev you can disable email confirmations in Supabase.)",
      );
      setPassword("");
      setConfirmPassword("");
    }
  ...
  return (
    <form className="flex flex-col gap-3" onSubmit={(e) => void onSubmit(e)} noValidate>
      ...
      <div>
        <label htmlFor={confirmId} className={formLabelClass}>
          Confirm password
        </label>
        <input
          id={confirmId}
      ...
      {message ? (
        <div role="status" className={messageBoxClass(tone)}>
          {message}
        </div>
      ) : null}
